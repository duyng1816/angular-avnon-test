...
export class FormBuilderComponent implements OnInit {
  @ViewChild('btnCloseQuestionModal', { static: true })
  btnCloseQuestionModal!: ElementRef;
  ...
  reviewAnswer(): void {
    this.router.navigate([
      'form/answer',
      { questions: JSON.stringify(this.listForm.value.questions) },
    ]);
  }
  ...
  requireValidator(control: AbstractControl): ValidationErrors | null {
    ...
    if (val.isRequired) {
      if (val.questionType === QuestionType.paragraph) {
        ...
        return val.answers.filter((x: any) => x.isSelected).length
          ? null
          : { required: true };
      }
    }
